    ...
    <div class="upload-container">
        <el-card class="upload-card">
            ...
            <div class="image-display">
                ...
                <!-- 流程完成提示 -->
                <div class="process-complete" v-if="processDone">
                    <el-result 
                        icon="success"
                        title="处理流程已全部完成"
                        sub-title="冠状动脉三维重建数据处理流程已全部完成"
                    >
                        <template #extra>
...
import { UploadFilled, Picture, PictureFilled, Download, Check } from '@element-plus/icons-vue'
import { ElMessage } from 'element-plus'
import { multiCenterProcess, testMultiCenterProcess } from '@/api/multiCenter'
...
onMounted(() => {
    if (route.query.from_previous === 'true') {
        ...
        if (route.query.imageUrls) {
            ...
            ElMessage.success(`已接收 ${receivedImageUrls.value.length} 张图像准备进行风格统一处理`)
        }
...
const handleUpload = async () => {
    ...
    try {
        ...
        if (testMode.value) {
            ...
            result = await testMultiCenterProcess();
        } else {
            ...
            result = await multiCenterProcess(formData, (progressEvent) => {
        ...
        if (result.code === 200 && result.data) {
            ...
            uploadResult.value = {
                type: true,
                message: `成功处理 ${resultList.value.length} 张图像！`,
        ...
        uploadResult.value = {
            type: false,
            message: '处理失败：' + (error.message || '服务器错误')
        }
        resultList.value = []
...
const finishProcess = () => {
    ...
    ElMessage.success('恭喜！冠状动脉三维重建数据处理流程已全部完成')
}

// 重新开始处理流程
const restartProcess = () => {
    // 重定向到第一个处理步骤
    router.push('/data-processing/extraction')
}
</script>
